...
import { EmailPasswordSignin } from "@/types/auth";
import Link from "next/link";
import { useState } from "react";
import { toast } from "sonner";

export default function SignIn() {
  const [form, setForm] = useState<EmailPasswordSignin>({
    email: "",
    password: "",
  });
  ...
  return (
    <div className="min-h-screen bg-[--card] flex items-center justify-center px-4">
      <Card className="w-full max-w-sm shadow-md border border-border">
        <CardHeader className="space-y-2 pb-0">
          <CardTitle className="text-xl font-semibold text-center mb-0">
            Welcome back
          </CardTitle>
        ...
        <CardContent className="space-y-4 pt-4">
          <form className="space-y-3" onSubmit={handleSignInWithPassword}>
